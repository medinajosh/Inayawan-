...
const BuildingPermitRequest = () => {
  ...
  const fetchRequests = async () => {
    const q = query(collection(firestore, "buildingPermitRequests"), orderBy("timestamp", "desc"));
  ...
  const handleDelete = async () => {
    if (requestToDelete) {
      await deleteDoc(doc(firestore, "buildingPermitRequests", requestToDelete));
      fetchRequests();
  ...
  const handleApproval = async (status) => {
    if (requestToApprove) {
      const requestRef = doc(firestore, "buildingPermitRequests", requestToApprove.id);
  ...
  return (
    <div className="p-8 bg-gradient-to-r from-blue-100 to-blue-300 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-4xl font-extrabold text-blue-800">🏗️ Building Permit Requests</h2>
      </div>
      ...
      {/* Building Permit request details modal */}
      {selectedRequest && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-50">
          <div className="absolute top-0 left-0 right-0 bottom-0 bg-[#000000a0] bg-opacity-50 z-40" />
          <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg max-w-md w-full z-50">
            <h3 className="text-3xl font-bold text-gray-800 mb-4">Building Permit Request</h3>
            <p className="text-lg text-gray-700"><strong>Project Name:</strong> {selectedRequest.projectName} </p>
            ...
            {selectedRequest.picture && <img src={selectedRequest.picture} alt="Building Permit Request" className="w-full h-48 object-contain rounded-lg mb-4" />}
...
export default BuildingPermitRequest;
