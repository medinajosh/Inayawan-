...
const Feedback = () => {
  ...
  const handleDelete = async () => {
    if (requestToDelete) {
      await deleteDoc(doc(firestore, "feedbacks", requestToDelete));
      fetchFeedback();
  ...
  const handleApproval = async (status) => {
    if (requestToApprove) {
      const requestRef = doc(firestore, "feedbacks", requestToApprove.id);
      await updateDoc(requestRef, { statusOfRequest: status });
      fetchFeedback();
      window.alert(`Feedback ${status}!`);
      setShowApprovalModal(false);
      setRequestToApprove(null);
    }
  };

  const filteredFeedback = feedback.filter((report) => {
  ...
  return (
    <div className="p-8 bg-gradient-to-r from-blue-100 to-blue-300 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-4xl font-extrabold text-blue-800">📩 Feedback</h2>
      </div>
      ...
      {selectedRequest && (
        ...
        <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg max-w-md w-full z-50">
            ...
            <p className="text-lg text-gray-700"><strong>Feedback:</strong> {selectedRequest.feedback}</p>
