...
const RequestInbox = () => {
  ...
  // Fetch all data from Firestore, ordered by timestamp descending
  const fetchRequests = async () => {
    const q = query(collection(firestore, "certificateRequests"), orderBy("timestamp", "desc"));
  ...
  const handleDelete = async () => {
    if (requestToDelete) {
      await deleteDoc(doc(firestore, "certificateRequests", requestToDelete));
      fetchRequests();
  ...
  const handleApproval = async (status) => {
    if (requestToApprove) {
      const requestRef = doc(firestore, "certificateRequests", requestToApprove.id);
  ...
  const filteredRequests = requests.filter((request) => {
    ...
    const status = request.statusOfRequest || "Pending"; // Default to "Pending" if status is missing
  ...
  return (
    <div className="p-8 bg-gradient-to-r from-blue-100 to-blue-300 min-h-screen">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-4xl font-extrabold text-blue-800">📩 Certificate Requests</h2>
      </div>
            ...
            {filteredRequests.length > 0 ? (
              filteredRequests.map((request) => (
                <tr key={request.id} className="border-b hover:bg-blue-50">
                  <td className="py-3 px-4">{request.firstName} {request.lastName}</td>
      ...
      {selectedRequest && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg max-w-md w-full z-50">
            ...
            <p className="text-lg text-gray-700"><strong>Name:</strong> {selectedRequest.firstName} {selectedRequest.middleName} {selectedRequest.lastName}</p>
