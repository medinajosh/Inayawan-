...
const RequestCertificate = ({ closeModal }) => {
  ...
  const fetchRequests = async () => {
    const querySnapshot = await getDocs(collection(firestore, "certificateRequests"));
    const requestData = querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id, timestamp: doc.data().timestamp?.toDate().toLocaleString() }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await addDoc(collection(firestore, "certificateRequests"), newRequest);
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center z-50">
      ...
      <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg max-w-md w-full z-50">
        ...
        <h2 className="text-2xl font-bold text-center text-blue-700 mb-6">Request a Certificate</h2>
        <p className="text-lg text-gray-600 mb-8 text-center">Fill in the details to request an official certificate.</p>
        ...
        <div className="overflow-y-auto max-h-100">
          ...
          <label className="block text-lg font-medium text-gray-700">First Name</label>
            <input
              ...
              required
            />
          <label className="block text-lg font-medium text-gray-700">Middle Name</label>  
            <input
              type="text"
              name="middleName"
          ...
          <label className="block text-lg font-medium text-gray-700">Email</label>
            ...
            <label className="block text-lg font-medium text-gray-700">Requirment</label>
              <label
                htmlFor="file-input"
